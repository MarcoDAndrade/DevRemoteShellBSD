...
Parse()
{
	...
	tcpdump -nr ${PCAPFile} 2> /dev/null | awk '{print $3,$5}' | grep '^[0-9]' | sed 's/:$//g' > ${listaIPs}
	...
	for IP in $IPs; do
		...
		# Total de portas
		grep " $IP" ${listaIPs} | awk '{print $2}' | cut -d. -f 5 | sort | uniq -c | sort -n > ${listaPort}
		TotalPortas=$(wc -l < ${listaPort})
...
trap
trap "rm $listaIPs $listaPort; echo 'Clear some files ;)'; exit" SIGHUP SIGINT SIGTERM
